...
public class LSTM {
	...
	public String[] run(double[] p_b_pre_arr) throws NumberFormatException, IOException {
		Random random = new Random(2016666);
		...
		// dataRead = new DataRead("test.txt");// datafile
		//double[] y_pre = dataRead.getData();
		double fix = Arrays.stream(p_b_pre_arr).sum() / p_b_pre_arr.length;
		Normalization normalization = new Normalization();
		double[] y = normalization.dataProcess(p_b_pre_arr, fix);
		...
		for (int cut_iter = 0; cut_iter < 50; ++cut_iter) {// default:1000
			...
			System.out.println("y_pred_normal = [" + String.join(",", predict) + "], ");
			// recurrent output
			...
			lstm.param.apply_diff(0.01);
			System.out.println("loss: " + loss);
			lstm.clear();
			if (cut_iter == 49)
			{System.out.println(String.join(",", predict_recur));
